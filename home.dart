...
class _HomeState extends State<Home> {
  ...
  Future showdialog(BuildContext context,String message,int total)async{
    ...
    child: new AlertDialog(
      ...
      actions: [
        new FlatButton(onPressed: null, child: Text('OK'))
      ],
    )
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Mess bill Calculator'),
                              ...
                              itemBuilder: (BuildContext context,
                                ...
                                return new ListTile(
                                  ...
                                  subtitle: TextField(
                                    onSubmitted: (value){
                                     if(index==0) {
                                      val=int.parse(num1.text)*int.parse(snapshot.value['Itemrate']);
                                      Add(val);
                                     } else if(index==1) {
                                       val = int.parse(num2.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     } else if(index==2)
                                       {
                                         val = int.parse(num3.text)*int.parse(snapshot.value['Itemrate']);
                                         Add(val);
                                       }
                                     else if(index==3)
                                     {
                                       val = int.parse(num4.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     }
                                     else if(index==4)
                                     {
                                       val = int.parse(num5.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     }
                                     else if(index==5)
                                     {
                                       val = int.parse(num6.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     }
                                     else if(index==6)
                                     {
                                       val = int.parse(num7.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     }
                                     else if(index==7)
                                     {
                                       val = int.parse(num8.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     }
                                     else if(index==8)
                                     {
                                       val = int.parse(num9.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     }
                                     else if(index==9)
                                     {
                                       val = int.parse(num10.text)*int.parse(snapshot.value['Itemrate']);
                                       Add(val);
                                     }
                                    },
                                    controller: index==0?num1:
                                        index==1?num2:
                                    index==2?num3:
                                    index==3?num4:
                                    index==4?num5:
        ...
        children: [
          ...
          SizedBox(
            ...
            child: FloatingActionButton(
              heroTag: 'btn2',
              onPressed: (){
                showdialog(context, "Backiam mess", Sum());
              },
              child: Icon(Icons.request_page,size: 30.0,),
            ),
          ),
        ],
      ),
